...
This is the guard the design promises in its §2.2 table and which nothing else implements. Without it a Service Worker directory arrives with `warnings: []`, so `applyGuards` computes `selected: true` and `gigabye --yes` deletes real offline app data with no interaction — the exact loss the rewrite exists to prevent. A scanner `note` does not do this job: `note` is dim descriptive text, `warnings` is what drives `selected`.
