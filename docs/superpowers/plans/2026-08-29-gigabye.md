# gigabye Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship `gigabye`, a macOS CLI that finds regenerable disk junk, shows it grouped and sized in an interactive checkbox review, and deletes only what the user leaves checked.
...
**Spec:** `docs/superpowers/specs/2026-08-29-gigabye-design.md`

## Global Constraints

Every task's requirements implicitly include this section.

- **Zero runtime dependencies.** `dependencies` in `package.json` must stay `{}`. Only `esbuild` and `typescript` are permitted in `devDependencies`.
...
- **Sizes come from `stat.blocks * 512`, never `stat.size`.** Violating this makes gigabye over-report freed space on iCloud-optimized files.
...
- Package name `gigabye`, binary name `gigabye`, repo `github.com/soummyaanon/gigabye`.
...
**Interfaces:**
- Consumes: nothing.
- Produces: `Candidate`, `Group`, `Warning`, `Verdict`, `RunManifest` types; `formatBytes(n: number): string`.
...
```json
{
  "name": "gigabye",
  "version": "0.1.0",
  "description": "Say gigabye to the build junk on your Mac.",
  "type": "module",
  "bin": { "gigabye": "./dist/gigabye.js" },
  "files": ["dist", "README.md", "LICENSE"],
  "engines": { "node": ">=22.18" },
  "os": ["darwin"],
  "license": "MIT",
  "author": "soummyaanon",
  "repository": { "type": "git", "url": "git+https://github.com/soummyaanon/gigabye.git" },
  "keywords": ["macos", "disk", "cleanup", "node_modules", "cache", "cli"],
  "scripts": {
    "test": "node --test 'test/**/*.test.ts'",
    "typecheck": "tsc --noEmit",
    "build": "esbuild src/cli.ts --bundle --platform=node --target=node22 --format=esm --outfile=dist/gigabye.js --banner:js='#!/usr/bin/env node'",
...
```bash
git add package.json tsconfig.json .gitignore LICENSE src/types.ts src/util/bytes.ts test/util/bytes.test.ts
...
The single most important correctness requirement in the spec. `du` uses allocated blocks; `stat.size` does not. An iCloud-optimized placeholder has a large `size` and zero `blocks`, so using `size` would make gigabye claim to free space that was never on disk.
...
/**
 * Directories never worth walking into. Walking these is slow, and nothing
 * inside them is ever a deletion candidate.
 */
...
const ctx: ScanContext = { home: '/Users/x', staleDays: 60, now: Date.now() }

function visit(p: string, entries: string[] = []): DirVisit {
  const parts = p.split('/')
  return { path: p, name: parts[parts.length - 1] as string, parent: parts.slice(0, -1).join('/'), entries }
}

test('claims unambiguous framework output directories', () => {
  for (const name of ['.next', '.turbo', '.parcel-cache', '.svelte-kit', '.astro']) {
    const got = buildsScanner.inspect(visit(`/Users/x/proj/${name}`), ctx)
...
test('claims dist only when a sibling manifest exists', () => {
  const withManifest = buildsScanner.inspect(
    { ...visit('/Users/x/proj/dist'), parent: '/Users/x/proj' },
    ctx,
  )
  assert.equal(withManifest, null, 'dist needs the parent manifest list to decide')
})

test('claims dist when the parent has a package.json', () => {
  const v: DirVisit = { path: '/Users/x/proj/dist', name: 'dist', parent: '/Users/x/proj', entries: [] }
  const got = buildsScanner.inspect(v, { ...ctx, parentEntries: ['package.json', 'dist'] } as ScanContext)
  assert.ok(got)
})

test('ignores build directories with no manifest beside them', () => {
  const v: DirVisit = { path: '/Users/x/docs/build', name: 'build', parent: '/Users/x/docs', entries: [] }
  const got = buildsScanner.inspect(v, { ...ctx, parentEntries: ['index.md'] } as ScanContext)
  assert.equal(got, null)
})

test('ignores directories it does not recognise', () => {
  assert.equal(buildsScanner.inspect(visit('/Users/x/proj/src'), ctx), null)
})
```

**Note to the implementer:** the two tests above reveal a design problem — `builds.ts` needs to know the *parent's* entries to decide about `dist`/`build`, but `DirVisit.entries` holds the *directory's own* entries. Fix this in Step 3 by adding `parentEntries: string[]` to `DirVisit` in `src/scan/walker.ts` and populating it in the walker, then rewrite these tests to use `parentEntries` on the visit rather than smuggling it through `ScanContext`. Do not ship the `as ScanContext` cast.
...
export async function walk(
  ...
  while (queue.length > 0) {
    ...
    for (const d of dirents) {
      ...
      if (d.name.endsWith('.photoslibrary')) continue
      if (NEVER_DESCEND.has(d.name)) continue
      queue.push({ dir: path.join(dir, d.name), parentEntries: entries })
    }
  }
}
```
...
export type WalkScanner = {
   ...
   */
  inspect(v: DirVisit, ctx: ScanContext): RawCandidate | null
}
...
- [ ] **Step 6: Rewrite the two casted tests to use `parentEntries`**

Replace the `as ScanContext` tests in `test/scan/builds.test.ts`:

```ts
test('claims dist when the parent has a package.json', () => {
  const got = buildsScanner.inspect(
    { path: '/Users/x/proj/dist', name: 'dist', parent: '/Users/x/proj', entries: [], parentEntries: ['package.json', 'dist'] },
    ctx,
  )
  assert.ok(got)
...
test('ignores build directories with no manifest beside them', () => {
  const got = buildsScanner.inspect(
    { path: '/Users/x/docs/build', name: 'build', parent: '/Users/x/docs', entries: [], parentEntries: ['index.md'] },
    ctx,
  )
  assert.equal(got, null)
})
...
Also update the `visit()` helper to include `parentEntries: []`, and delete the now-redundant `claims dist only when a sibling manifest exists` test.

- [ ] **Step 7: Run the tests to verify they pass**

Run: `npm test && npm run typecheck`
Expected: PASS. Typecheck clean — no `as ScanContext` casts remain.
...
`nodeModulesScanner` needs the filesystem (to check recent activity), so unlike `buildsScanner` its `inspect` is async. Widen the contract in `src/scan/scanner.ts`:

```ts
inspect(v: DirVisit, ctx: ScanContext): RawCandidate | null | Promise<RawCandidate | null>
```
...
const ctx: ScanContext = { home: '/Users/x', staleDays: 60, now: NOW }
...
test('claims cargo target only beside a Cargo.toml', async () => {
  const yes = cargoScanner.inspect(visit('/Users/x/rs/target', { parentEntries: ['Cargo.toml', 'src'] }), ctx)
  assert.ok(yes)
  assert.equal(yes.label, 'target (cargo)')
  const no = cargoScanner.inspect(visit('/Users/x/misc/target', { parentEntries: ['notes.md'] }), ctx)
  assert.equal(no, null)
})

test('claims a python venv only when it contains pyvenv.cfg', () => {
  const yes = pythonScanner.inspect(visit('/Users/x/py/.venv', { entries: ['pyvenv.cfg', 'bin'] }), ctx)
  assert.ok(yes)
  const no = pythonScanner.inspect(visit('/Users/x/py/env', { entries: ['main.py'] }), ctx)
  assert.equal(no, null)
})
```
 ...
 * .git. Only the top two levels are checked — deep enough to catch src/,
 * shallow enough to stay fast. Mirrors the -maxdepth 2 heuristic in diskdiet.
 */
...
export const nodeModulesScanner: WalkScanner = {
  ...
  async inspect(v, ctx) {
    ...
    const touched = await lastTouched(v.parent)
...
async function fakeHome(dirs: string[]): Promise<ScanContext> {
  ...
  return { home, staleDays: 60, now: Date.now() }
}
...
const TARGETS: Array<[string, string, string | undefined]> = [
  ...
  ['Library/Developer/Xcode/iOS DeviceSupport', 'iOS DeviceSupport', 're-created on device connect'],
  ['Library/Developer/Xcode/watchOS DeviceSupport', 'watchOS DeviceSupport', undefined],
...
**Interfaces:**
- Consumes: `PathScanner`, `ScanContext`.
- Produces: `orphansScanner: PathScanner`. Candidates carry `group: 'orphans'`, which Task 12's guard layer blocks from ever being selectable.
...
async function home(dirs: string[]): Promise<ScanContext> {
  ...
  return { home: h, staleDays: 60, now: Date.now() }
}
...
**Note to the implementer:** these tests pass on a machine where Slack is not installed and fail on one where it is. Make `applicationDirs` an optional third field on `ScanContext` (defaulting to `['/Applications', path.join(home, 'Applications')]`) and have the test pass an empty array, so the result never depends on the developer's machine. Add `applicationDirs: string[]` to `ScanContext` in `src/scan/scanner.ts` and set the default in Task 18's wiring.
...
- [ ] **Step 4: Add `applicationDirs` to `ScanContext`**

In `src/scan/scanner.ts`:
...
export type ScanContext = {
  ...
  staleDays: number
  now: number
  /** Where to look for installed .app bundles. Injected for testability. */
  applicationDirs: string[]
}
```

Update every existing test's `ctx` literal to include `applicationDirs: []`.
...
```bash
git add src/scan/orphans.ts src/scan/scanner.ts test/
...
The guard layer is the reason gigabye is safe on a machine nobody has inspected. A guard may **block** (never selectable, never deletable) or **downgrade** (shown unchecked, with a visible warning).

**Files:**
- Create: `src/guard/guard.ts` (contract), `src/guard/home.ts`, `src/guard/symlink.ts`, `src/guard/volume.ts`, `src/guard/sync-roots.ts`, `src/guard/keep.ts`, `src/guard/orphan.ts`
...
- Produces:
  ...
  - `homeGuard`, `symlinkGuard`, `volumeGuard`, `syncRootsGuard`, `keepGuard`, `orphanGuard`
...
const HOME = '/Users/x'
const ctx: GuardContext = { home: HOME, homeDev: 1, keepGlobs: [] }
...
export type GuardContext = {
  ...
  homeDev: number
  /** Glob patterns from ~/.gigabyerc that must never be touched. */
  keepGlobs: string[]
}
...
export const syncRootsGuard: Guard = {
  ...
  check(c, ctx) {
    ...
    for (const rel of SYNC_ROOTS) {
      const root = path.join(path.resolve(ctx.home), rel)
      if (resolved === root || resolved.startsWith(root + path.sep)) {
        return { action: 'downgrade', warning: 'syncs to your other machines' }
      }
    }
    return ALLOW
  },
...
test('INVARIANT: nothing outside home ever survives the guard pipeline', async () => {
  ...
  const survivors = await applyGuards(outside, { home, homeDev, keepGlobs: [] })
...
test('allows an ordinary candidate inside home', async () => {
  ...
  const [got] = await applyGuards([cand(target)], { home, homeDev, keepGlobs: [] })
  assert.ok(got)
...
test('block wins over downgrade', async () => {
  ...
  const survivors = await applyGuards([cand(target)], { home, homeDev, keepGlobs: ['**/keepme/**'] })
...
test('drops orphans from the selectable list', async () => {
  ...
  const survivors = await applyGuards([cand(target, { group: 'orphans' })], { home, homeDev, keepGlobs: [] })
...
const GUARDS: Guard[] = [
  homeGuard, keepGuard, orphanGuard, syncRootsGuard, symlinkGuard, volumeGuard, gitGuard,
]
...
async function sandbox() {
  ...
  return { home, homeDev: st.dev, runsDir, ctx: { home, homeDev: st.dev, keepGlobs: [] } }
}
...
export async function reap(
  ...
  // Re-run the guards. Anything that no longer passes is silently skipped.
  const revalidated = await applyGuards(wanted, ctx)
  const cleared = new Set(revalidated.map((r) => r.path))

  const reaped: ReapedItem[] = []
  for (const item of wanted) {
    if (!cleared.has(item.path)) continue
    try {
...
**Note to the implementer:** the re-validation deliberately runs `applyGuards` again rather than reusing the scan-time verdict. `applyGuards` drops blocked candidates and marks downgraded ones `selected: false`, but the reaper only checks membership in `cleared`, not the fresh `selected` flag — a downgraded item the user deliberately checked must still be deletable. That is intended. Do not "fix" it by filtering on `r.selected`.
...
git commit -m "feat: argument parsing and ~/.gigabyerc config"
```
...
- Produces:
  - `renderReport(items: Reviewed[], opts: { color: boolean }): string`
  - `renderJson(items: Reviewed[]): string`
...
test('emits no ANSI escapes when color is off', () => {
  const out = renderReport([item()], NO_COLOR)
  assert.ok(!/\[/.test(out), 'ANSI escapes present with color disabled')
})
...
function palette(color: boolean) {
  ...
  const wrap = (code: string): Paint => (s) => `[${code}m${s}[0m`
...
export function renderReport(items: Reviewed[], opts: { color: boolean }): string {
  ...
  for (const group of ORDER) {
    ...
    for (const i of inGroup) {
      ...
      lines.push(`  ${box} ${formatBytes(i.bytes).padStart(9)}  ${c.cyan(tildify(i.path))}${note}${warn}`)
    }
  }
...
Run: `npm test && npm run typecheck`
Expected: PASS. Note the tilde test passes `/Users/x` paths while `os.homedir()` differs on the test machine — make `renderReport` accept the home via `tildify(i.path, ...)`. If the test fails for this reason, thread a `home` field through the `opts` object (`{ color: boolean; home: string }`) and update the test to pass `home: '/Users/x'`.
...
export function renderFrame(
  ...
  const paint = opts.color
    ? (code: string, str: string) => `[${code}m${str}[0m`
...
export function renderHistory(runs: RunManifest[], opts: { color: boolean }): string {
  const dim = opts.color ? (s: string) => `[2m${s}[0m` : (s: string) => s
  const bold = opts.color ? (s: string) => `[1m${s}[0m` : (s: string) => s
...
git commit -m "feat: gigabye history"
```
...
async function main(argv: string[]): Promise<number> {
  ...
  if (opts.command === 'history') {
    ...
    process.stdout.write(opts.json ? `${JSON.stringify(shown, null, 2)}\n` : renderHistory(shown, { color }))
    return 0
  }
  ...
  const guardCtx: GuardContext = { home, homeDev: homeStat.dev, keepGlobs: config.keep }
...
- [ ] **Step 4: Reconcile `renderReport`'s signature**

Task 14 left `renderReport(items, { color })`; `cli.ts` calls it with `{ color, home }`. Update `src/ui/render.ts` so both `renderReport` and `renderJson` take `{ color: boolean; home: string }` and pass `home` to `tildify`, then update `test/ui/render.test.ts` to pass `home: '/Users/x'`. Run `npm run typecheck` to confirm no other caller is left behind.
...
```bash
npm run build
node dist/gigabye.js --help
node dist/gigabye.js --dry-run builds
```
    ...
    steps:
      ...
      - name: zero runtime dependencies
        run: |
          ...
          if [ "$deps" != "{}" ]; then
            echo "gigabye must have no runtime dependencies, found: $deps"
            exit 1
          fi
...
```bash
npm login
gh repo create soummyaanon/gigabye --public --source=. --push
npm version 0.1.0 --no-git-tag-version
git commit -am "chore: 0.1.0" && git tag v0.1.0 && git push --tags
```
...
**Known reconciliations, deliberately left in the plan rather than hidden.**
Three tasks change an interface a previous task defined. Each says so
explicitly and updates its callers in the same task, because that is what
actually happens when you build this:

- Task 4 adds `parentEntries` to `DirVisit` (Task 3).
- Task 5 widens `WalkScanner.inspect` to allow a promise (Task 4).
- Task 7 adds `applicationDirs` to `ScanContext` (Task 4).
- Task 9 adds `gitIgnored` to `GuardContext` (Task 8).
- Task 17 Step 4 adds `home` to `renderReport`'s options (Task 14).
